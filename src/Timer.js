...
const Timer = () => {
  ...
  const [tempoPassado, setTempoPassado] = useState(false);
  ...
  const getTempo = () => {
      ...
      if (tempo >= 0) {
          setTempoPassado(false);
          setDias(Math.floor(tempo / (1000 * 60 * 60 * 24)));
          setHoras(Math.floor((tempo / (1000 * 60 * 60)) % 24));
          setMinutos(Math.floor((tempo / 1000 / 60) % 60));
          setSegundos(Math.floor((tempo / 1000) % 60));
      } else {
          setTempoPassado(true);
          const tempoPositivo = Math.abs(tempo);
          setDias(Math.floor(tempoPositivo / (1000 * 60 * 60 * 24)));
          setHoras(Math.floor((tempoPositivo / (1000 * 60 * 60)) % 24));
          setMinutos(Math.floor((tempoPositivo / 1000 / 60) % 60));
          setSegundos(Math.floor((tempoPositivo / 1000) % 60));
      }
  };

  useEffect(() => {
      const intervalo = setInterval(() => getTempo(), 1000);
